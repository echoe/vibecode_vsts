...
juce::AudioProcessorValueTreeState::ParameterLayout FMPluginAudioProcessor::createParameterLayout()
{
    ...
    juce::StringArray waveChoices { "Sine", "Triangle", "Saw", "Square" };

    // Generate parameters for each operator dynamically
    for (int i = 0; i < FMVoice::numOperators; ++i)
    {
    ...
    for (int src = 0; src < FMVoice::numOperators; ++src)
    {
        for (int dest = 0; dest < FMVoice::numOperators; ++dest)
        {
